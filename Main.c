#include <stdio.h>
#include <string.h>
#include <sys/param.h>
#include <netinet/in.h>
#include "math.h"
#include <errno.h> 
#include <fcntl.h> 

// Includes de FreeRTOS y Sistema
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
#include "esp_system.h"
#include "esp_event.h"
#include "esp_log.h"
#include "nvs_flash.h"
#include "esp_task.h"
#include "esp_timer.h" // Para esp_timer_get_time()

// Includes de Red (Wi-Fi y Sockets)
#include "esp_wifi.h"
#include "esp_netif.h"
#include "lwip/sockets.h"
#include "lwip/netdb.h"
#include "lwip/inet.h"

// Includes de Drivers (I2C)
#include "driver/gpio.h"
#include "driver/i2c_master.h"
#include "sdkconfig.h" 


// ---- Configuración del TCP Client ----
#define TAG "BMI_TCP_CLIENT"
#define WIFI_SSID       CONFIG_WIFI_SSID
#define WIFI_PASS       CONFIG_WIFI_PASSWORD
#define SERVER_IP       CONFIG_SERVER_IP
#define SERVER_PORT     CONFIG_SERVER_PORT

// ---- Configuración del BMI270 ----
#define I2C_MASTER_SCL_IO				GPIO_NUM_47				//GPIO pin
#define I2C_MASTER_SDA_IO				GPIO_NUM_48				//GPIO pin
#define I2C_MASTER_FREQ_HZ				100000					// Frecuencia I2C (100kHz)
#define ESP_SLAVE_ADDR					0x68					// Dirección I2C del BMI270

// ---- Globales BMI270 ----
esp_err_t ret = ESP_OK;
i2c_master_dev_handle_t device;

// ---- Variables de Estado Globales ----
static bool is_sending_data = false;
static int current_frequency = 800; // Frecuencia inicial por defecto
const int DEFAULT_FREQ = 800;

/*! @name  Global array that stores the configuration file of BMI270 (8192 bytes) */
const uint8_t bmi270_config_file[] = {
    
};

// ===============================================
// === FUNCIONES DE LÓGICA DEL BMI270 (I2C) ======
// ===============================================

esp_err_t bmi_read( uint8_t *data_address, uint8_t *data_rd, size_t size) {
    if (size == 0) {
        return ESP_OK;
    }
    esp_err_t ret;
    ret = i2c_master_transmit(device, data_address, 1, pdMS_TO_TICKS(1000));
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Fallo en I2C transmit (lectura): %s", esp_err_to_name(ret));
        return ret;
    }
    ret = i2c_master_receive(device, data_rd, size, pdMS_TO_TICKS(1000));
     if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Fallo en I2C receive: %s", esp_err_to_name(ret));
    }
    return ret;
}

esp_err_t bmi_write(uint8_t *data_address, uint8_t *data_wr, size_t size) {
    esp_err_t ret;
    uint8_t *full_data = malloc(1 + size);
    if (full_data == NULL) {
        ESP_LOGE(TAG, "Fallo al alocar memoria para buffer I2C");
        return ESP_ERR_NO_MEM;
    }
    full_data[0] = *data_address;
    memcpy(&full_data[1], data_wr, size);
    ret = i2c_master_transmit(device, full_data, 1 + size, pdMS_TO_TICKS(1000));
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Fallo en I2C transmit (escritura): %s", esp_err_to_name(ret));
    }
    free(full_data);
    return ret;
}

esp_err_t bmi_init() {
    esp_err_t ret;
    ESP_LOGI(TAG, "Inicializando bus I2C...");
    i2c_master_bus_config_t i2c_mst_config = {
        .clk_source = I2C_CLK_SRC_DEFAULT,
        .i2c_port = I2C_NUM_0,
        .scl_io_num = I2C_MASTER_SCL_IO,
        .sda_io_num = I2C_MASTER_SDA_IO,
        .glitch_ignore_cnt = 7,
        .flags.enable_internal_pullup = true,
    };
    i2c_master_bus_handle_t bus_handle;
    ESP_ERROR_CHECK(i2c_new_master_bus(&i2c_mst_config, &bus_handle));
    i2c_device_config_t dev_cfg = {
        .dev_addr_length = I2C_ADDR_BIT_LEN_7,
        .device_address = ESP_SLAVE_ADDR,
        .scl_speed_hz = I2C_MASTER_FREQ_HZ,
    };
    ESP_LOGI(TAG, "Añadiendo dispositivo BMI270 en 0x%X", ESP_SLAVE_ADDR);
    ret = i2c_master_bus_add_device(bus_handle, &dev_cfg, &device);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Fallo al añadir dispositivo I2C: %s", esp_err_to_name(ret));
    }
    return ret;
}

void chipid(void) {
    uint8_t reg_id=0x00;
    uint8_t tmp;
    bmi_read( &reg_id, &tmp, 1);
    printf("Valor de CHIPID: 0x%02X \n", tmp);
    if(tmp == 0x24){
        printf("Chip reconocido.\n\n");
    }
    if(tmp != 0x24) {
        printf("Chip no reconocido. \nCHIP ID: 0x%02X\n\n", tmp);
        vTaskDelay(portMAX_DELAY);
    }
}

void softreset(void) {
    uint8_t reg_softreset=0x7E, val_softreset = 0xB6;
    ret = bmi_write( &reg_softreset, &val_softreset, 1);
    vTaskDelay(pdMS_TO_TICKS(1000)); 
    if(ret != ESP_OK){
        printf("\nError en softreset: %s \n",esp_err_to_name(ret));
    }
    else {
         printf("\nSoftreset: OK\n\n");
    }
}

void initialization(void) {
    uint8_t reg_pwr_conf_advpowersave=0x7C, val_pwr_conf_advpowersave=0x00;
    uint8_t reg_init_ctrl=0x59, val_init_ctrl=0x00, val_init_ctrl2=0x01;
    uint8_t reg_init_data=0x5E;

    printf("Inicializando ...\n");
    bmi_write( &reg_pwr_conf_advpowersave, &val_pwr_conf_advpowersave, 1);
    printf("Esperando 1s para estabilizar sensor...\n");
    vTaskDelay(pdMS_TO_TICKS(1000));
    ret=bmi_write( &reg_init_ctrl, &val_init_ctrl, 1);
    if(ret != ESP_OK) {
        printf("\nError en init_ctrl=0\n");
    }
    int config_size = sizeof(bmi270_config_file);
    printf("Comenzando carga de config_file (%d bytes) en bloque unico...\n", config_size);
    ret=bmi_write( &reg_init_data, (uint8_t*)bmi270_config_file, config_size);
    if(ret != ESP_OK) {
        printf("\nError cargando config_file: %s\n", esp_err_to_name(ret));
    }
    else {
        printf("\nConfig_file cargado.\n");
    }
    printf("Esperando 1s para procesar firmware...\n");
    vTaskDelay(pdMS_TO_TICKS(1000));
    ret=bmi_write( &reg_init_ctrl, &val_init_ctrl2, 1);
    printf("\nAlgoritmo de inicialización finalizado.\n\n");
}

void check_initialization(void){
    uint8_t reg_internalstatus=0x21;
    uint8_t tmp;
    vTaskDelay(pdMS_TO_TICKS(1000)); 
    bmi_read( &reg_internalstatus, &tmp, 1);
    uint8_t init_status = tmp & 0b00000011; 
    printf("Internal Status (Reg 0x21): 0x%02X\n", tmp);
    if (init_status == 0x01) {
        printf("Comprobación Inicialización: OK (Status: 0x01)\n\n");
    } else {
        printf("Inicialización fallida (Status: 0x%02X)\n\n", init_status);
        vTaskDelay(portMAX_DELAY);
    }
}

void internal_status(void)
{
    uint8_t reg_internalstatus=0x21;
    uint8_t tmp;
    bmi_read( &reg_internalstatus, &tmp, 1);
    printf("Internal Status: 0x%02X\n\n", tmp);
}

// --- CORRECCIÓN --- Volver a la lógica "Accel-Only"
void bmipowermode(int Fodr)
{
    // --- Habilitar SOLO Accel (0x04) ---
    uint8_t reg_pwr_ctrl=0x7D, val_pwr_ctrl=0x04; // 0x04 = Accel ON, Gyro OFF
    uint8_t reg_acc_conf=0x40, val_acc_conf; 
    uint8_t reg_pwr_conf=0x7C, val_pwr_conf=0x00; // Modo normal
    
    // --- Configuración de ACCEL ---
    // Rango +/-8g (0b0010) y Filtro normal (0b10) -> 0x2_
    // ODR: 400Hz (0xA), 800Hz (0xB), 1600Hz (0xC)
    if(Fodr==400) val_acc_conf=0x2A;  
    else if(Fodr==800) val_acc_conf=0x2B;
    else if(Fodr==1600) val_acc_conf=0x2C;  
    else {
        printf("FRECUENCIA ACCEL (%d Hz) INCORRECTA. Usando 800 Hz.\n", Fodr);
        val_acc_conf = 0x2B;
        current_frequency = 800;
    }

    printf("Configurando modo de energia y ODR a %d Hz...\n", Fodr);
    // 1. Configurar ACC (ODR y Rango)
    bmi_write( &reg_acc_conf, &val_acc_conf, 1);
    vTaskDelay(pdMS_TO_TICKS(10)); 
    // 2. Salir de modo suspendido (si aplica)
    bmi_write( &reg_pwr_conf, &val_pwr_conf, 1);
    vTaskDelay(pdMS_TO_TICKS(10));
    // 3. Encender el sensor (último paso)
    bmi_write( &reg_pwr_ctrl, &val_pwr_ctrl, 1);

    printf("Esperando 1s para estabilizar ODR...\n");
    vTaskDelay( 1000 /portTICK_PERIOD_MS);
    
    current_frequency = Fodr; // Confirmar cambio de estado
}

static void handle_set_frequency(int sock, int new_freq) {
    if (new_freq != 400 && new_freq != 800 && new_freq != 1600) {
        ESP_LOGW(TAG, "Frecuencia solicitada inválida: %d Hz", new_freq);
        return;
    }
    ESP_LOGI(TAG, "Cambiando frecuencia a %d Hz...", new_freq);
    bmipowermode(new_freq);
    char ack_msg[32];
    int len = snprintf(ack_msg, sizeof(ack_msg), "ACK,FREQ_%d\n", current_frequency);
    if (send(sock, ack_msg, len, 0) < 0) {
        ESP_LOGW(TAG, "Error enviando ACK de frecuencia");
    }
    ESP_LOGI(TAG, "ACK de frecuencia enviado.");
}


// ===============================================
// === FUNCIONES DE LÓGICA DE RED (Wi-Fi/TCP) ====
// ===============================================

// (wifi_event_handler, wifi_init_sta, connect_tcp - sin cambios)
static void wifi_event_handler(void *arg, esp_event_base_t event_base,
                               int32_t event_id, void *event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        esp_wifi_connect();
    } else if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_DISCONNECTED) {
        ESP_LOGW(TAG, "WiFi desconectado, reintentando...");
        esp_wifi_connect();
    } else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP) {
        ip_event_got_ip_t *event = (ip_event_got_ip_t *)event_data;
        ESP_LOGI(TAG, "IP obtenida: " IPSTR, IP2STR(&event->ip_info.ip));
    }
}

static void wifi_init_sta(void)
{
    ESP_ERROR_CHECK(esp_netif_init());
    ESP_ERROR_CHECK(esp_event_loop_create_default());
    esp_netif_create_default_wifi_sta();
    wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
    ESP_ERROR_CHECK(esp_wifi_init(&cfg));
    ESP_ERROR_CHECK(esp_event_handler_instance_register(
        WIFI_EVENT, ESP_EVENT_ANY_ID, &wifi_event_handler, NULL, NULL));
    ESP_ERROR_CHECK(esp_event_handler_instance_register(
        IP_EVENT, IP_EVENT_STA_GOT_IP, &wifi_event_handler, NULL, NULL));
    wifi_config_t wifi_config = {0};
    strcpy((char *)wifi_config.sta.ssid, WIFI_SSID);
    strcpy((char *)wifi_config.sta.password, WIFI_PASS);
    ESP_LOGI(TAG, "Configurando WiFi en modo STA...");
    ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_STA));
    ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_STA, &wifi_config));
    ESP_ERROR_CHECK(esp_wifi_start());
    ESP_LOGI(TAG, "Conectando a SSID:%s ...", WIFI_SSID);
}

static int connect_tcp(void)
{
    int sock = -1;
    struct sockaddr_in serv_addr = {0};
    serv_addr.sin_family = AF_INET;
    serv_addr.sin_port   = htons(SERVER_PORT);
    if (inet_pton(AF_INET, SERVER_IP, &serv_addr.sin_addr) <= 0) {
        ESP_LOGE(TAG, "Dirección IP inválida: %s", SERVER_IP);
        return -1;
    }
    sock = socket(AF_INET, SOCK_STREAM, 0);
    if (sock < 0) {
        ESP_LOGE(TAG, "socket() falló");
        return -1;
    }
    ESP_LOGI(TAG, "Conectando a %s:%d ...", SERVER_IP, SERVER_PORT);
    if (connect(sock, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
        ESP_LOGE(TAG, "connect() falló: %s", strerror(errno));
        close(sock);
        return -1;
    }
    ESP_LOGI(TAG, "Conectado a %s:%d", SERVER_IP, SERVER_PORT);
    return sock;
}

// ===============================================
// === TAREA PRINCIPAL (LÓGICA DE CONTROL CORREGIDA) =
// ===============================================

static char rx_buffer[1024];
static int rx_buffer_len = 0;

void process_commands(int sock) {
    char *cmd_start = rx_buffer;
    char *cmd_end;
    
    while ((cmd_end = strchr(cmd_start, '\n')) != NULL) {
        *cmd_end = '\0'; 
        char *command = cmd_start;
        cmd_start = cmd_end + 1; 

        int len = strlen(command);
        if (len > 0 && command[len - 1] == '\r') {
            command[len - 1] = '\0';
        }
        
        if (strlen(command) == 0) {
            continue;
        }

        ESP_LOGI(TAG, "Comando procesado: [%s]", command);

        if (strcmp(command, "CMD,START") == 0) {
            is_sending_data = true;
            ESP_LOGW(TAG, "Estado: Iniciando envío de datos.");
        } 
        else if (strcmp(command, "CMD,STOP") == 0) {
            is_sending_data = false;
            ESP_LOGW(TAG, "Estado: Deteniendo envío de datos.");
        } 
        else if (strncmp(command, "CMD,SET_FREQ,", 13) == 0) {
            int new_freq = atoi(command + 13);
            handle_set_frequency(sock, new_freq);
        }
        else {
            ESP_LOGW(TAG, "Comando desconocido: [%s]", command);
        }
    }
    
    int remaining_len = strlen(cmd_start);
    memmove(rx_buffer, cmd_start, remaining_len);
    rx_buffer_len = remaining_len;
    rx_buffer[rx_buffer_len] = '\0'; 
}

static void sensor_tcp_task(void *arg)
{
    uint8_t reg_status = 0x03; // Registro STATUS (ACC_DRDY está en el bit 7)
    uint8_t reg_data = 0x0C; // Registro de inicio de datos (ACC_X_LSB)
    uint8_t data_data8[6]; 
    uint16_t acc_x, acc_y, acc_z;
    char tx_buffer[256];

    // Variables de Batching
    const int SAMPLES_PER_BATCH = 20; // Enviar cada 20 muestras (a 800Hz, 40 envíos/seg)
    char tx_batch_buffer[3072]; // Buffer grande para 20 muestras
    char tx_line_buffer[160]; // Buffer para una sola línea
    int samples_in_batch = 0;

    int64_t last_heartbeat_time = 0;
    const int64_t heartbeat_interval_ms = 1000;

    tx_batch_buffer[0] = '\0'; // Inicializar el buffer de lote

    for (;;) {
        int sock = -1;
        while ((sock = connect_tcp()) < 0) {
            vTaskDelay(pdMS_TO_TICKS(2000)); 
        }

        handle_set_frequency(sock, current_frequency); 
        is_sending_data = false; 
        rx_buffer_len = 0; 
        last_heartbeat_time = esp_timer_get_time() / 1000;
        
        int flags = fcntl(sock, F_GETFL, 0);
        fcntl(sock, F_SETFL, flags | O_NONBLOCK);

        tx_batch_buffer[0] = '\0'; // Limpiar buffer al (re)conectar
        samples_in_batch = 0;

        while (1) {
            
            // --- A. Leer Comandos (No bloqueante) ---
            int len = recv(sock, rx_buffer + rx_buffer_len, sizeof(rx_buffer) - 1 - rx_buffer_len, 0);
            if (len > 0) {
                rx_buffer_len += len;
                rx_buffer[rx_buffer_len] = '\0';
                process_commands(sock);
            } 
            else if (len == 0) {
                ESP_LOGW(TAG, "Servidor cerró la conexión.");
                break;
            } 
            else {
                if (errno != EWOULDBLOCK && errno != EAGAIN) {
                    ESP_LOGE(TAG, "Error en recv(): %s", strerror(errno));
                    break;
                }
            }
            
            // --- B. Lógica de Estado (Enviar Datos o Heartbeat) ---
            if (is_sending_data) {
                // --- MODO: ENVIANDO DATOS ---
                
                uint8_t status_reg_val;
                bmi_read(&reg_status, &status_reg_val, 1);
                
                if ((status_reg_val & 0x80) == 0x80) // ACC_DRDY
                { 
                    ret = bmi_read( &reg_data, (uint8_t*) data_data8, 6);
                    if(ret != ESP_OK){
                        ESP_LOGW(TAG, "Error lectura BMI: %s", esp_err_to_name(ret));
                        continue; 
                    }

                    // Parsear y formatear
                    acc_x = ((uint16_t) data_data8[1] << 8) | (uint16_t) data_data8[0];
                    acc_y = ((uint16_t) data_data8[3] << 8) | (uint16_t) data_data8[2];
                    acc_z = ((uint16_t) data_data8[5] << 8) | (uint16_t) data_data8[4];
                    float f_acc_x = (int16_t)acc_x * (8.000 / 32768.0);
                    float f_acc_y = (int16_t)acc_y * (8.000 / 32768.0);
                    float f_acc_z = (int16_t)acc_z * (8.000 / 32768.0);
                    int line_len = snprintf(tx_line_buffer, sizeof(tx_line_buffer), 
                                     "DATA,%.4f,%.4f,%.4f,0.0,0.0,0.0\n",
                                     f_acc_x, f_acc_y, f_acc_z);

                    // Añadir al lote
                    if ((strlen(tx_batch_buffer) + line_len) < sizeof(tx_batch_buffer)) {
                        strcat(tx_batch_buffer, tx_line_buffer); 
                        samples_in_batch++;
                    }

                    // Enviar lote si está lleno
                    if (samples_in_batch >= SAMPLES_PER_BATCH) {
                        if (send(sock, tx_batch_buffer, strlen(tx_batch_buffer), 0) < 0) {
                            if (errno != EWOULDBLOCK && errno != EAGAIN) {
                                ESP_LOGE(TAG, "Error en send() de lote: %s", strerror(errno));
                                break; 
                            }
                        } else {
                            tx_batch_buffer[0] = '\0';
                            samples_in_batch = 0;
                        }
                    }
                }
            } 
            else {
                // --- MODO: INACTIVO (ENVIAR HEARTBEAT) ---
                int64_t now = esp_timer_get_time() / 1000;
                if (now - last_heartbeat_time > heartbeat_interval_ms) {
                    last_heartbeat_time = now;
                    const char *heartbeat_msg = "STATUS,IDLE\n";
                    if (send(sock, heartbeat_msg, strlen(heartbeat_msg), 0) < 0) {
                         if (errno != EWOULDBLOCK && errno != EAGAIN) {
                             ESP_LOGE(TAG, "Error en send() de heartbeat: %s", strerror(errno));
                             break;
                        }
                    }
                }
            }

            // --- C. CEDER CPU (LA CORRECCIÓN MÁS IMPORTANTE) ---
            // Dormir por 1 tick (que ahora es 1ms gracias al menuconfig).
            // Esto garantiza que el stack de red (lwIP) y otras tareas
            // tengan tiempo de ejecutarse, sin importar si estamos
            // enviando o inactivos.
            vTaskDelay(pdMS_TO_TICKS(1)); 
        }

        // --- Limpieza al desconectar ---
        if (sock >= 0) {
            ESP_LOGW(TAG, "Cerrando socket.");
            close(sock);
        }
        vTaskDelay(pdMS_TO_TICKS(1000));
    }
}

// ===============================================
// ================= APP_MAIN ====================
// ===============================================

void app_main(void)
{
    // 1. Inicializar NVS
    esp_err_t ret = nvs_flash_init();
    if (ret == ESP_ERR_NVS_NO_FREE_PAGES || ret == ESP_ERR_NVS_NEW_VERSION_FOUND) {
        ESP_ERROR_CHECK(nvs_flash_erase());
        ESP_ERROR_CHECK(nvs_flash_init());
    }

    // 2. Inicializar Wi-Fi
    wifi_init_sta();

    // 3. Inicializar I2C y Sensor BMI270
    ESP_LOGI(TAG, "--- Iniciando BMI270 ---");
    ESP_ERROR_CHECK(bmi_init());
    softreset();
    chipid();
    initialization();
    check_initialization();
    bmipowermode(DEFAULT_FREQ); // Iniciar a 800 Hz
    internal_status();    
    ESP_LOGI(TAG, "--- BMI270 Listo ---");

    // 4. Crear la tarea de control TCP
    xTaskCreatePinnedToCore(
        sensor_tcp_task,    
        "sensor_tcp_task",  
        8192,               
        NULL,               
        5,                  
        NULL,               
        tskNO_AFFINITY      
    );
    
    ESP_LOGI(TAG, "app_main finalizado, la tarea de sensor/TCP está corriendo.");
}